...
export const legalPacks=[
  {
    ...
    reason:'Ce texte doit être lu avec les autres règles qui organisent l’astreinte, l’intervention et le repos.'
  },
  {
    id:'temps',
    label:'Temps de travail & repos',
    icon:'🕒',
    ...
    refs:['PERS77'],
    titles:['accord national sur le temps de travail','accord temps de travail','note enedis grdf temps de travail','repos quotidien de 11 heures','astreinte et intervention de sécurité gaz'],
    reason:'Cette règle se combine avec les accords de temps de travail et, lorsqu’il y a astreinte, avec les règles de repos et d’intervention.'
  },
  {
    id:'repas',
    label:'Repas & déplacements',
    icon:'🍽️',
    triggers:['repas','déplacement','deplacement','frais','11 h et 13 h','11h 13h','18 h et 21 h','18h 21h','cantine'],
    refs:['PERS793'],
    titles:['guide idf des élus cse-e','guide idf des elus cse-e'],
    reason:'La PERS 793 fixe le droit de base ; la jurisprudence et les règles de déplacement peuvent préciser sa preuve et son application.'
  },
  {
    id:'discipline',
    label:'Discipline & procédure',
    icon:'⚠️',
    triggers:['discipline','sanction','avertissement','blâme','blame','ep1','ep2','courrier de rappel','faute','culture juste'],
    refs:['PERS846'],
    titles:['règlement intérieur grdf','reglement interieur grdf','culture juste'],
  ...
  {
    id:'mandats',
    label:'CSE, CSSCT & mandat',
    icon:'🗣️',
    triggers:['cse','cssct','proxi','élu','elu','mandat','délégation','delegation','réunion employeur','reunion employeur'],
    refs:[],
    titles:['guide idf des élus cse-e','guide idf des elus cse-e'],
    reason:'Le guide pratique doit être rapproché des textes de niveau supérieur qui fondent le droit du mandat.'
  },
  {
    id:'sante',
...
export function packsForQuery(query){
  ...
  if(/\bpers\s*530\b|\bpers530\b|\bpers\s*557\b|\bpers557\b|\bpers\s*849\b|\bpers849\b|\bpers\s*939\b|\bpers939\b/.test(q)&&!found.some(p=>p.id==='astreinte')) found.push(legalPacks.find(p=>p.id==='astreinte'));
  if(/\bpers\s*77\b|\bpers77\b/.test(q)&&!found.some(p=>p.id==='temps')) found.push(legalPacks.find(p=>p.id==='temps'));
  if(/\bpers\s*793\b|\bpers793\b/.test(q)&&!found.some(p=>p.id==='repas')) found.push(legalPacks.find(p=>p.id==='repas'));
  if(/\bpers\s*846\b|\bpers846\b/.test(q)&&!found.some(p=>p.id==='discipline')) found.push(legalPacks.find(p=>p.id==='discipline'));
