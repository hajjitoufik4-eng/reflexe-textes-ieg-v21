...
function LawBridge({items}){
 if(!items.length)return null;
 return <section className="law-bridge">
   <div className="ieg-bridge-head"><div className="ieg-bridge-icon">📘</div><div><span>Droit commun</span><h2>Les règles légales à connaître d’abord</h2><p>Avant les PERS et les accords GRDF, voici les articles du Code du travail et le cadre européen qui structurent le sujet.</p></div></div>
   <div className="law-grid">{items.map(x=><a className="law-card" href={x.url} target="_blank" rel="noopener noreferrer" key={x.id}><div><span>{x.level}</span><b>{x.ref}</b></div><strong>{x.title}</strong><p>{x.simple}</p><small><strong>Pourquoi ici :</strong> {x.why}</small><i>Ouvrir la source officielle ↗</i></a>)}</div>
 </section>;
}
...
export default async function Catalogue({ params, searchParams }) {
  ...
  const lawItems=scope==='grdf'&&relationQuery?publicLawFor(relationQuery):[];
  ...
  if(scope==='grdf'){
    ...
    iegAssociated=unique([...byPack,...byQuery,...byTheme,...essentials]).sort((a,b)=>Number(isExtension(a.d))-Number(isExtension(b.d))||a.priority-b.priority||Number(explanationFor(b.d).mode==='verified')-Number(explanationFor(a.d).mode==='verified')||String(a.d.ref||a.d.title).localeCompare(String(b.d.ref||b.d.title),'fr'));
  ...
  return <>
    ...
    <section className={`catalogue-head catalogue-${scope}`}>
      ...
      <div><span className="section-kicker">{scope==='ieg'?'Statut & branche':'Entreprise GRDF'}</span><h1>{topic ? topic[1] : scope==='ieg'?'Les textes IEG, rangés simplement.':'Tes règles GRDF, avec tous les textes qui les encadrent.'}</h1><p>{topic?topic[2]:scope==='ieg'?`${total} références indexées. Tu peux chercher avec des mots normaux : repas, astreinte, sanction, congé…`:`Cherche un sujet comme « astreinte ». Le site reconstruit le bloc complet : droit commun + PERS / notes IEG + accords, décisions et notes GRDF.`}</p></div>
    </section>
    ...
    {!theme&&!query?<>
      <div className="catalogue-help"><strong>Je cherche par situation</strong><span>Choisis une carte : le site ajoutera automatiquement le droit commun et le socle IEG correspondant.</span></div>
      ...
      <div className="catalogue-results-head"><Link className="back" href={catalogueUrl(scope,{level})}>← Tous les thèmes</Link><span>{scope==='grdf'?`${lawItems.length} droit commun + ${iegAssociated.length} IEG + ${grdfItems.length} GRDF`:`${directList.length} résultat${directList.length>1?'s':''}`}</span></div>

      {scope==='grdf'&&<LawBridge items={lawItems}/>} 
