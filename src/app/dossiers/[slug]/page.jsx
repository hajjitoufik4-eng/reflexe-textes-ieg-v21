...
export default async function Dossier({params}){
 ...
 return <>
  ...
  <section id="jurisprudence" className="reader"><h2>Jurisprudence vérifiée rattachée au dossier ({cases.length})</h2>{cases.map(j=><article className="case" key={j.id}><h3>{j.court} — {j.date} · n° {j.number}</h3><p><strong>Question :</strong> {j.issue}</p><p><strong>Rendu :</strong> {j.result}</p><p><strong>Portée pratique :</strong> {j.scope}</p>{j.judgment?.amounts?.length>0&&<div className="amounts">{j.judgment.amounts.map(([label,amount])=><div className="amount-row" key={label}><span>{label}</span><strong>{amount}</strong></div>)}</div>}<a className="source" href={j.url} target="_blank" rel="noopener noreferrer">Décision source ↗</a></article>)}</section>
 </>;
}
