...
export default async function Document({ params }) {
  ...
  const d = data.find((x) => x.id === id); if (!d) notFound();
  ...
  return <>
    ...
    {important && <p className="notice"><strong>Texte structurant</strong> — cette fiche réunit le texte source, son application pratique, les textes liés et la jurisprudence pertinente. Les décisions externes sont distinguées des règles propres aux IEG/GRDF.</p>}
    <section className="reader"><nav className="tabs" aria-label="Sections de la fiche"><a href="#essentiel">L’essentiel</a><a href="#regles">Règles</a><a href="#articulation">Articulation</a><a href="#jurisprudence">Jurisprudence ({jurisprudence.length})</a><a href="#original">Original</a></nav>
      <section id="essentiel"><h2>Ce qu’il faut comprendre</h2>{x ? <div className="simple">{x.simple}</div> : <p>Le document est présent dans le corpus. L’analyse détaillée doit être validée sur le texte original avant publication.</p>}</section>
      ...
      <section id="jurisprudence"><h2>Jurisprudence et décisions</h2>{jurisprudence.length ? jurisprudence.map((j,i)=><article className="case" key={j.id || i}><h3>{j.court || j.juridiction} — {j.date}{j.number || j.numero ? ` · n° ${j.number || j.numero}` : ''}</h3>{j.issue && <p><strong>Question tranchée :</strong> {j.issue}</p>}{j.result && <p><strong>Résultat du jugement :</strong> {j.result}</p>}{j.scope && <p><strong>Ce que cela change concrètement :</strong> {j.scope}</p>}{j.url && <a className="source" href={j.url} target="_blank" rel="noopener noreferrer">Consulter la décision officielle ↗</a>}</article>) : <p className="muted">Aucune décision vérifiée n’est encore rattachée à cette fiche.</p>}</section>
      <section id="original"><h2>Document original</h2>{url ? <><a className="source" href={url} target="_blank" rel="noopener noreferrer">{isLocalPdf(url) ? 'Ouvrir le PDF ↗' : 'Consulter la source ↗'}</a>{isLocalPdf(url) && <><a className="source" href={url} download>Télécharger</a><object className="pdf" type="application/pdf" data={url} aria-label={'Document original : '+d.title}><p>Utilisez « Ouvrir le PDF » si le document ne s’affiche pas.</p></object></>}</> : <p>L’original n’est pas encore relié à cette fiche.</p>}</section>
    </section>
  </>;
}
